...
namespace Decoder
{
    public class PlaneSheremeteva_Peleha : Plane
    {
        ...
        public PlaneSheremeteva_Peleha(int N, int M, int P, double n, double m, double p) : base(N, M, P, n, m, p)//
        {
        ...
        public void SymetriGamX(double[,] GamX, double r)
        {
            bool sim = true;
            for (int i = 0; i < GamX.GetLength(1) ; i++)
                for (int j = 0; j < GamX.GetLength(0)/2; j++)
                    if (GamX[j, i] + GamX[GamX.GetLength(0)-1-j, i] >r) { sim = false; Console.WriteLine("d " + i + "  " + j + "  "+ (GamX[j, i] + GamX[GamX.GetLength(0) - 1 - j, i])); }
           
            
            Console.WriteLine("sim = " + sim);
        }
        public void SymetriGamY(double[,] GamY, double r)
        {
            bool sim = true;
            for (int i = 0; i < GamY.GetLength(0); i++)
                for (int j = 0; j < GamY.GetLength(1) / 2; j++)
                    if (GamY[i, j] + GamY[i, GamY.GetLength(1) - 1 - j] > r) { sim = false; Console.WriteLine("d " + i + "  " + j + "  " + (GamY[i, j] + GamY[i, GamY.GetLength(1) - 1 - j])); }
            ...
            if (Z0 != 0)
                if (FlagMultiModularIntensiveDeformations)
                    return (double)(2.0 * Z0 *
                        Math.Sqrt(Tet * Math.Pow(Z0 * DGamxX - Z0 * Z0 * (DGamxX + d2wx) / 3 + lam0 * (DGamyY - Z0 * Z0 * (DGamyY + d2wy) / 3), 2.0) -
                    ...
                    //return (double)(2.0/3 * Math.Sqrt(Tet * Math.Pow(Z0* DGamxX - Z0 * Z0 * Z0 / (3 * p)* (DGamxX + d2wx) + (Z0 * DGamyY - Z0 * Z0 * Z0 / (3 * p) * (DGamyY + d2wy)), 2.0)
                    //    - 3 * (Z0*DGamxX - Z0 * Z0 * Z0 / (3 * p) * (DGamxX + d2wx)) * (Z0*DGamyY - Z0 * Z0 * Z0 / (3 * p) * (DGamyY + d2wy))
                    //    + 3 / 2.0 * (Math.Pow(Z0*(DGamxY + DGamyX) - Z0 * Z0 * Z0 / (3 * p) * (DGamxY + DGamyX + d2wxy), 2.0)+
                    //return (double)(2.0 / 3 * Math.Abs(Z0) *
                    //    Math.Sqrt(Tet * Math.Pow(DGamxX - Z0*Z0*DGamxX/(3)  + DGamyY - Z0 * Z0 * DGamyY/(3)
                    //    - d2wx - d2wy, 2.0)));


                    return (double)(2.0 / 3 *
                        Math.Sqrt(Tet * Math.Pow(Z0 * DGamxX + Z0 * DGamyY - Z0 * Z0 * Z0 / 3 * (DGamxX + d2wx) - Z0 * Z0 * Z0 / 3 * (DGamyY + d2wy), 2.0) +
                        -3 * (Z0 * DGamxX - Z0 * Z0 * Z0 * (DGamxX + d2wx) / 3) * (Z0 * DGamyY - Z0 * Z0 * Z0 * (DGamyY + d2wy) / 3) +
                        3 / 2.0 * (Math.Pow(Z0 * (DGamyX + DGamxY) - Z0 * Z0 * Z0 / 3 * (DGamyX + DGamxY + d2wxy), 2.0)



            ////Math.Pow((gamy[x + 1, y + 1] + dwy) * Fun[x, y] * fun(Z0), 2.0) +
            //lam0* lam01*Math.Pow((gamy[x + 1, y + 1] + dwy) * Fun[x, y] * fun(Z0), 2.0) +

            ////Math.Pow((gamx[x + 1, y + 1] + dwx) * Fun[x, y] * fun(Z0), 2.0)));
            //lam01 * Math.Pow((gamx[x + 1, y + 1] + dwx) * Fun[x, y] * fun(Z0), 2.0))));
        ...
        public int MaximumCountIterationOfDecisionSystems = 0; // Суммарное число всех итераци прощитанных в системе
            ...
            int BorderExitMethodABS, int BorderExitPhisicalIteration, int TypeBorder1, int TypeBorder2, int TypeBorder3, int TypeBorder4)
        {
            ...
            MaximumCountIterationOfDecisionSystems = 0;
        ...
        private double Aij(double[,] XY, int x, int y, bool xy, double[,] F, int Ai, int Aj)
        {
            ...
            switch ((Ai, Aj))
            {
                ...
                case (0, 1):
                    ...
                    if (xy) return 2 * Math.Pow(XY[0,x + 1], 2.0) * DM2dy * Math.Pow(lam, 4.0)/3;
                ...
                case (0, 9):
                    if (xy)
                    {
                        return (double)Math.Pow(lam, 4.0) * XY[2, x + 1] * XY[0, x + 1] * DM1[x, y];
                    } 
                    else
                    { 
                        return 0;
                ...
                case (0, 10):
                    ...
                    if (xy)
                    {
                        return XY[0, x + 1]* (double)Math.Pow(lam, 4.0) * 2 * XY[2, x + 1] * DM1dy;
                    }
                    else
                    {
                ...
                case (0, 11):
                    ...
                    if (xy)
                    {
                        ...
                        return lam*lam*XY[0, x + 1] * (lam * lam * XY[2, x + 1] * (DM1d2y + RM1d2x/(lam*lam)) + XY2d2x * (RM1[x, y] + SM1[x, y]) +
                            XY2dx * (2 * RM1dx + SM1dx));
                    }
                    else
                    {
                ...
                case (0, 12):
                    ...
                    else
                    {
                        ...
                        XY2dy = (XY[2, y + 2] - XY[2, y]) / (2 * dy);
                        return Math.Pow(lam,4.0)*XY[0, y + 1] * (XY2d3y * DM1[x, y] + 2 * XY2d2y * DM1dy + XY2dy * (DM1d2y +  RM1d2x/(lam * lam)));
                    }
                ...
                case (2, 2):
                    ...
                    {
                        XY0dy = (XY[0, y + 2] - XY[0, y]) / (2 * dy);
                        return XY[2, y + 1]  * XY0dy * SM2dx / (3*lam* lam);
                    }
                ...
                case (2, 5):
                    ...
                    {
                        return XY[2, y + 1] / (lam * lam) * XY[2, y + 1] * SM1dx;
        ...
        private void Pn(double[,,] AXnYn, double[,] XnYn, bool xy, double[,] F)
        {
            ...
            for (int Ai = 0; Ai < AXnYn.GetLength(0); Ai++)
            {
                for (int Aj = 0; Aj < AXnYn.GetLength(1); Aj++) 
                {
                        ...
                        for (int j = 1; j < jLength; j=j+2)
                        {
                            ...
                            if (xy)
                            {
                                ...
                                if (j == jLength - 1)
                                {
                                    ...
                                    f1 = Aij(XnYn, j, i+2, xy, F, Ai, Aj);
                                    f2 = Aij(XnYn, j + 1, i+2, xy, F, Ai, Aj);
                                    AXnYn[Ai, Aj, i] = AXnYn[Ai, Aj, i] + (f1 + f2) * dx / 2;
                                }
                                else
                                {
                                    f1 = Aij(XnYn, j, i+2, xy, F, Ai, Aj);
                                    f2 = Aij(XnYn, j+1, i+2, xy, F, Ai, Aj);
                                    f3 = Aij(XnYn, j+2, i+2, xy, F, Ai, Aj);
                                    AXnYn[Ai, Aj, i] = AXnYn[Ai, Aj, i] + (f1 + 4 * f2 + f3) * dx / 3;
                                }
                            }
                            else
                            {
                                ...
                                {
                                    //формула трапеции на случай если количество точек разбиение чётное
                                    f1 = Aij(XnYn, i+2, j , xy, F, Ai, Aj);
                                    f2 = Aij(XnYn, i+2 , j + 1, xy, F, Ai, Aj);
                                    AXnYn[Ai, Aj, i] = AXnYn[Ai, Aj, i] + (f1 + f2) * dx / 2;
                                }
                                else
                                {
                                    f1 = Aij(XnYn, i+2, j , xy, F, Ai, Aj);
                                    f2 = Aij(XnYn, i+2 , j + 1, xy, F, Ai, Aj);
                                    f3 = Aij(XnYn, i+2 , j + 2, xy, F, Ai, Aj);
                                    AXnYn[Ai, Aj, i] = AXnYn[Ai, Aj, i] + (f1 + 4 * f2 + f3) * dx / 3;
                                }
                            }
                        }
        ...
        private double[,] SolutionSystemsProblemMetodGaussaForVariationIteration(double[,] XY,double d, double[,,] A, int GrUsl1, int GrUsl2, bool xy,double eps)
        {
            ...
            while (true)
            {
                ...
                //ShowMassiv2(FXnYn);
                MaximumCountIterationOfDecisionSystems++;
                //if (MaxDifferents < eps) break;
                break;
            }
            //ShowMassiv2(XY);
            //Console.WriteLine(MaxDifferents);
            return XY;
        }
        ...
        private void MethodVariationIteration(Load F, int TypeBorder1, int TypeBorder2, int TypeBorder3, int TypeBorder4)
        {
            ...
            for (int x = 0; x < N + 4; x++)
            {
                Xn[0,x] = (double)(Math.Sin(Math.PI * dx * (x - 2)) * Math.Sin(Math.PI * dy * (x - 2)));
                Xn[1, x] = (double)(Math.Sin(2*Math.PI * dx * (x - 2)));
                Xn[2, x] = 1;
            }
            for (int y = 0; y < M + 4; y++)
            {
            ...
            for (int k = 1; k <= BorderExitIterationMethods; k++)
            {
                ...
                Yn = SolutionSystemsProblemMetodGaussaForVariationIteration(Yn, dy, A, TypeBorder4, TypeBorder2,true,0.00000001);
                ...
                Pn(A, Yn, false, F.F);
                // находим решение уравнения методом гаусса жордана
                Xn = SolutionSystemsProblemMetodGaussaForVariationIteration(Xn, dx, A, TypeBorder4, TypeBorder2, false, 0.00000001);
                // относительно граничного условия и кубической интерполяцией задаём заграничные значения.
                ...
                //Console.WriteLine(" i "+ MaxWForMethodIteration1);
                //ShowMassiv2(W);
                //ShowMassiv2(gamx);
                //ShowMassiv2(gamy);
            }
        ...
        private void PhisicalNolenearyPart(Load F, int TypeBorder1, int TypeBorder2, int TypeBorder3, int TypeBorder4, bool FlagABS, DelegatForMethods Method)
        {
            ...
            for (int fizi = 1; fizi <= BorderExitPhisicalIteration; fizi++)
            {
                ...
                //Console.WriteLine(MaxWForMethodIteration1);
                //ShowMassiv2(W);
                //Console.WriteLine(mW+" "+ CountIterationPhisicalNoneleneary+" "+ MaximumCountIterationOfDecisionSystems);
                // пересчёт всех значений с новыми прогибами
                //ShowMassiv2(W);
                //Symetri(W, 0.000001);
                //ShowMassiv2(gamx);
                //SymetriGamX(gamx, 0.000001);
                //ShowMassiv2(gamy);
                //SymetriGamY(gamy, 0.000001);
                //for (int i = 0; i < gamx.GetLength(0); i++)
                //    for (int j = 0; j < gamx.GetLength(1); j++)
                //        if (gamx[i, j] - gamy[j, i] > 0.0000001) Console.WriteLine(i + " " + j + " " + (gamx[i, j] - gamy[j, i]) + " gam ");
                Load();
