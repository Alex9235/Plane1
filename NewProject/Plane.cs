...
namespace Decoder
{
    public class Plane
    {
        ...
        public void InicializationTempature(String PathTemp, String PathDeformMap, double alf, double Tem0, double Tem1, double Def0, double Def1, double a)
        {
            // 
            if (FlagСurvilinearPlane)
            {
        ...
        public void InicializationHumidity(double QWater, double bet)
        {
            if (FlagСurvilinearPlane)
            {
                ErrorEnterConstructorParametrs();
        ...
        public void InicializationVariationGradient(double ConstRaspredGradient, double NU1, double E1, double alf1)
        {
            ...
            this.GradientE = E1 / G0;
            this.GradientNU = NU1;
            this.GradientAlf = alf1;

        }
        //Инициализация пористости
        public void InicializationPorisoty(double CountPorisoty, int ModelPorisoty)
        {
            if (CountPorisoty <= 0 || CountPorisoty > 1 || ModelPorisoty < 1 || ModelPorisoty > 4 || FlagTemperatureProblem)
            {
                ErrorEnterConstructorParametrs();
                return;
            }
        ...
        private void LoadTFULL(double ConstTemp)
        {
            for (int x = 0; x < N; x++)
                for (int y = 0; y < M; y++)
                    for (int z = 0; z < P; z++)
                        T[x, y, z] = ConstTemp * alf[x, y, z] * lam1 * lam1;
        }
            ...
            for (int z = 0; z < P; z++)
                ...
                for (int y = 0; y < M; y++)
                {
                    ...
                    foreach (var number in s.Split())
                    {
                        if (number != "")
                        {
                            T[x, y, z] = Convert.ToDouble(number) * alf[x, y, z] * r * a;
                            x++;
                        }
                    }
                }
                ...
                for (int y = 0; y < M; y++)
                    for (int z = 0; z < P; z++)
                    {
                        ...
                        if (FlagPorysotyProblem)
                            alf[x, y, z] = alf[x, y, z] * PoristFunctionToHeigth(dx * x, dy * y, DZ[x, y] * (z - (P - 1) / 2));
                        if (FlagGradientProblem)
                            alf[x, y, z] = (GradientAlf + (alf[x, y, z] - GradientAlf) *
        ...
        private void LoadCFULL(double ConstTemp)
        {
            for (int x = 0; x < N; x++)
                for (int y = 0; y < M; y++)
                    for (int z = 0; z < P; z++)
                        if(z>=P-3)
                            C[x, y, z] = ConstTemp * beta[x, y, z] * lam1 * lam1;
        }
            ...
            for (int x = 0; x < N; x++)
                for (int y = 0; y < M; y++)
                    for (int z = 0; z < P; z++)
                    {
                        ...
                        if (FlagPorysotyProblem)
                            beta[x, y, z] = beta[x, y, z] * PoristFunctionToHeigth(dx * x, dy * y, DZ[x, y] * (z - (P - 1) / 2));
                        if (FlagGradientProblem)
                            beta[x, y, z] = (GradientBeta + (beta[x, y, z] - GradientBeta) *
                            Math.Pow(1.0 / 2 + (DZ[x, y] * (z - ((P - 1) / 2)) / (2 * (1 + FunctionСurvilinearPlane(dx * x, dy * y)))), ConstRaspredGradient));
        ...
        protected void PoritostbLoadNU()
        {
            for (int x = 0; x < N; x++)
                for (int y = 0; y < M; y++)
                    for (int z = 0; z < P; z++)
